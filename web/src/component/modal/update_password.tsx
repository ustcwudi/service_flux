...
export default (props: any) => {
  ...
  return (
    <ModalForm visible={true} onCancel={props.onCancel} onFinish={() => updateRequest()}>
      <Grid item xs={12}>
        <TextField
          fullWidth
          type="password"
          ...
          label={'旧密码'}
          onChange={(e) => {
            setModel({ ...model, old_password: e.target.value })
          }}
        />
      </Grid>
      <Grid item xs={6}>
      ...
      <Grid item xs={6}>
        <TextField
          ...
          label={'重复'}
          onChange={(e) => {
            setModel({ ...model, re_password: e.target.value })
          }}
        />
      </Grid>
      <Grid item xs={12}>
